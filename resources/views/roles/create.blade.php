        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-6">
                <div class="bg-secondary rounded h-100 p-4">
                    ...
                    <form id="roleForm" action="{{ isset($role) ? route('roles.update', ['role' => $role->id]) : route('roles.store') }}" method="POST">
                        @csrf
                                            ...
                                            <td>
                                                ...
                                                @if (checkPermission($permissionGroup, 'create_' . strtolower($index)))
                                                    <div class="form-check text-center">
                                                        <input class="form-check-input" type="checkbox" name="permissions[]" value="create_{{ strtolower($index) }}" {{ isset($role) && $role->hasPermissionTo('create_' . strtolower($index)) ? 'checked' : '' }} style="margin-left: 2px;"  >
                                                    </div>
                                                @else
